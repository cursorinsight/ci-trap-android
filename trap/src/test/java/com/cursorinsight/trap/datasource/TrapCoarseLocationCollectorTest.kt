...
class TrapCoarseLocationCollectorTest {
    ...
    fun `test permission check`(@MockK activity: Activity) {
        ...
        val result = TrapCoarseLocationCollector.checkPermissions(activity)
        assert(result)
    }
    ...
    fun `test require permission when permission is already given`(@MockK activity: Activity) {
        ...
        var succeeded = false
        TrapCoarseLocationCollector.requirePermissions(activity) { succeeded = true }
